...
/**
 * Implementación del servicio para la gestión de vehículos. Esta clase sigue
...
public class VehicleImpl implements IVehicle {
	...
	/**
	 * Guarda un nuevo vehículo en la base de datos a partir de los datos
	 * proporcionados en un VehicleDto.
	 *
	...
	public Vehicle save(VehicleDto vehicleDto) {
		// Convierte los datos del VehicleDto en un objeto Vehicle siguiendo el
		// principio de abstracción adecuada.
		Vehicle vehicle = Vehicle.builder().id(vehicleDto.getId()).placa(vehicleDto.getPlaca())
				.modelo(vehicleDto.getModelo()).capacidad(vehicleDto.getCapacidad()).build();

		// Guarda el vehículo en la base de datos utilizando el VehicleDao, respetando
